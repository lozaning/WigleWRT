...
SESSION_DIR="$WIGLEWRT_BASE/sessions"
CURRENT_SESSION="$SESSION_DIR/current.csv"
SESSION_STATE="$WIGLEWRT_TMP/session_state.json"
...
# Write PID file (simple version for compatibility)
write_pid() {
    local pidfile="$1"
    echo $$ > "$pidfile"
}

# Write PID file atomically (prevents race conditions)
write_pid_atomic() {
    local pidfile="$1"
...
# Acquire exclusive lock using mkdir (atomic on Unix)
...
cleanup_stale_pids() {
    ...
    for pidfile in "$piddir"/*.pid; do
        ...
        if [ -n "$pid" ] && ! kill -0 "$pid" 2>/dev/null; then
            rm -f "$pidfile"
            log debug "Removed stale PID file: $pidfile"
        fi
    done
}
